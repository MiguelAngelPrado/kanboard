<section id="main">
    ...
    <form id="project-creation-form" method="post" action="<?= $this->url->href('ProjectCreationController', 'save') ?>" autocomplete="off">
        ...
        <?= $this->form->label(t('Nombre(s)'), 'name') ?>
        <?= $this->form->text('name', $values, $errors, array('autofocus', 'required')) ?>

        <?= $this->form->label(t('Apellidos'), 'apellidos') ?>
        <?= $this->form->text('apellidos', $values, $errors, array('autofocus', 'required')) ?>

        <?= $this->form->label(t('Identificador'), 'identifier') ?>
